...
int webserver_register_file(const char *path, const char *content_type) {
  ...
  if (rc) {
    Log_error("webserver", "Could not stat '%s': %s", local_fname,
              strerror(errno));
    return -1;
  }
...
static VD_GET_INFO_CALLBACK(webserver_get_info, filename, info, cookie) {
  ...
  while (virtfile != NULL) {
    if (strcmp(filename, virtfile->virtual_fname) == 0) {
      ...
      Log_info("webserver", "Access %s (%s) len=%zd", filename, contentType,
               virtfile->len);
      return 0;
    }
  ...
  Log_info("webserver",
           "404 Not found. (attempt to access "
           "non-existent '%s')",
           filename);
...
static VD_OPEN_CALLBACK(webserver_open, filename, mode, cookie) {
  if (mode != UPNP_READ) {
    Log_error("webserver", "%s: ignoring request to open file for writing.",
              filename);
    return NULL;
  }
...
static VD_READ_CALLBACK(webserver_read, fh, buf, buflen, cookie) {
  ...
  if (len < 0) {
    Log_error("webserver", "In %s: %s", __FUNCTION__, strerror(errno));
...
static VD_SEEK_CALLBACK(webserver_seek, fh, offset, origin, cookie) {
  ...
  if (newpos < 0 || newpos > (off_t)file->len) {
    Log_error("webserver", "in %s: seek failed with %s", __FUNCTION__,
              strerror(errno));
    return -1;
  }
...
static struct UpnpVirtualDirCallbacks virtual_dir_callbacks = {
    webserver_get_info, webserver_open, webserver_read,
    webserver_write,    webserver_seek, webserver_close};
...
#else
// With version 1.6.7 and above, the UPNP library maintainers made a
// questionable API choice to register the callbacks one-by-one, instead of
// having a struct with the callbacks which is certainly a better and 'typical'
// C API choice. They removed the UpnpVirtualDirCallbacks in the course of that.
