  ...
  public:
    ...
    */
    void apply(std::set<std::string>& types)
    {
      ...
      for (auto& mime_type : this->added_types)
        types.insert(mime_type);

      // Manually remove specific MIME types
      for (auto& mime_type : this->removed_types)
        types.erase(mime_type);
    }
    ...
    */
    void filter_by_root(std::set<std::string>& types)
    {
      ...
      while (it != types.end())
      {
        ...
        // Attempt to find the type in the allowed lists by matching the shortest string
        auto result = std::find_if(this->allowed_roots.begin(), this->allowed_roots.end(), [type](const std::string& root){
